...
AddEventHandler("np-commands:checkCommands", function(pCommands)
    commands = pCommands
    print(json.encode(pCommands[2]))
end)
        ...
        IsWhiteListedJudge(charID, function(judge)
            for k,v in pairs(commands) do
                if isSafe(police,ems,doctor,judge,driving,v[3],job) then
                    commandExport:AddCommandValid(v[1],v[2],v[3])
                end
            end
        end)
    end)
...
function IsWhiteListed(hexId, characterId, callback)
    ...
    exports["np-db"]:execute(q,v, function(results)
