...
Queue.InitHostName = Queue.InitHostName ~= "riddlev" and Queue.InitHostName or false
...
local function playerConnect(name, setKickReason, deferrals)
    ...
    local pSteamId = getIdentifier(self, source, "steam")
    local pLicense = getIdentifier(self, source, "license")
    local pLicense2 = getIdentifier(self, source, "license2")
    local pXbox = getIdentifier(self, source, "xbox")
    local pLive = getIdentifier(self, source, "live")
    local pDiscord = getIdentifier(self, source, "discord")
    local pCFX = getIdentifier(self, source, "fivem")
	...
	for i = 1, #BanList, 1 do
		if ((tostring(BanList[i].license)) == tostring(pLicense)
			...
			or (tostring(BanList[i].live)) == tostring(pLive) 
			or (tostring(BanList[i].discord)) == tostring(pDiscord) 
			or (tostring(BanList[i].ip)) == tostring(pIp) 
			or (tostring(BanList[i].token)) == tostring(pTokenData)
			or (tostring(BanList[i].cfx)) == tostring(pCFX))
		then
			print('hy')

			if (tonumber(BanList[i].expire)) > os.time() then

				local tempsrestant     = (((tonumber(BanList[i].expire)) - os.time())/60)
				if tempsrestant >= 1440 then
					...
					local txtminutes = math.ceil(minutes)
						done("You are banned, you may appeal it at discord.gg/hydeck | Ban ID: " .. BanList[i].banid .. " | Reason: " .. BanList[i].reason .." | Unban Date: "..  os.date('%Y-%m-%d', BanList[i].expire))					
						CancelEvent()
						break
			...
			elseif (tonumber(BanList[i].expire)) < os.time() and tonumber(BanList[i].expire) > 0 then
				break
			elseif tonumber(BanList[i].expire) <= 0 then
				done("You are banned, you may appeal it at discord.gg/hydeck | Ban ID: " .. BanList[i].banid .. " | Reason: " .. BanList[i].reason .." | Unban Date: Permanent") 
				CancelEvent() 
				break 
			end
		end
    ...
    while true do
        ...
        if Queue.DisplayQueue then
            if Queue.InitHostName then
                ...
                Queue.InitHostName = GetConvar("sv_hostname")
                Queue.InitHostName = Queue.InitHostName ~= "Nolife | Development" and Queue.InitHostName or false
            end
        end
    end
end)
...
if Config.DisableHardCap then
    Queue:DebugPrint("^1 [np-queue] Disabling hardcap ^7")
