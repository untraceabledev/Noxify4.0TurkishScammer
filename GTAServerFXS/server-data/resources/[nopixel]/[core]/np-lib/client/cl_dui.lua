...
function getDui(url, width, height)
    ...
    return {id = duiCounter, dictionary = generatedDictName, texture = generatedTxtName, duiHandle = duiObject}
end
...
function sendDuiMessage(id, data)
    if (not duis[id]) then
        return
    end

    local settings = duis[id]
    SendDuiMessage(settings.duiObject, json.encode(data))
end

function getDuiHandle (id)
    if (not duis[id]) then
        return
    end

    local settings = duis[id]
    return settings.duiObject
end

exports('getDui', getDui)
exports('changeDuiUrl', changeDuiUrl)
exports('releaseDui', releaseDui)
exports('sendDuiMessage', sendDuiMessage)
exports('getDuiHandle', getDuiHandle)
