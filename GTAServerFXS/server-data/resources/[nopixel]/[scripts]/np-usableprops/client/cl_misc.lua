...
local itemHandlers = {
    ...
    ['letterbox'] = function(info)
        ...
        if elements then
            ...
            if mailType == 'property' then
                ...
                if #(checkCoords - plyCoords) > 25.0 then
            ...
            elseif mailType == 'business' then
                ...
                local objects = exports["np-objects"]:GetVisibleObjects("objects")
        ...
        local results = exports['np-objects']:PlaceObject(
            ...
            function (coords, materialHash, entity)
                ...
                return roll < 25.0 and roll > -25.0 and pitch < 25.0 and pitch > -25.0 and dist < 25.0 and not onRoad
            end
        )
    ...
    ['cryptominer'] = function(info, invName, itemSlot)
        RPC.execute("np-usableprops:checkCryptoMiner", info, itemSlot)
        return false
    end,
    ...
    ['glassbox'] = function(info)
        local results = exports['np-objects']:PlaceObject(
            `np_building_concrete_xl`,
            {
        ...
        local heading = results[2].rotation.z

        local wallOffsets = {
            { x = 1.0, y = 1.82, z = 1.82, h = 0.0 },
            { x = -1.0, y = 1.82, z = 1.82, h = 0.0 },
            { x = 1.0, y = -1.82, z = 1.82, h = 0.0 },
            { x = -1.0, y = -1.82, z = 1.82, h = 0.0 },
            { x = 1.82, y = 1.0, z = 1.82, h = 90.0 },
            { x = 1.82, y = -1.0, z = 1.82, h = 90.0 },
            { x = -1.82, y = 1.0, z = 1.82, h = 90.0 },
            { x = -1.82, y = -1.0, z = 1.82, h = 90.0 }
        }
        ...
        local positions = {}
        for _,offset in pairs(wallOffsets) do
            local coords = GetObjectOffsetFromCoords(coords.x + 0.0, coords.y + 0.0, coords.z + 0.0, heading, offset.x, offset.y, offset.z)
            local rotation = { x = 0.0, y = 0.0, z = heading + offset.h }
            local position = { coords = coords, rotation = rotation }
            positions[#positions + 1] = position
        end

        RPC.execute('np-usableprops:placeGlassBox', coords, heading, positions)
        return false
    end,
    ...
    ['xmas_tree'] = function(info)
        local meta = json.decode(info)
        local modelMap = {
            ['Decorated'] = 'gabz_xmas_tree_01',
            ['Undecorated'] = 'gabz_xmas_tree_02',
        }
        local model = modelMap[meta.Style]
        local id = exports['np-objects']:PlaceAndSaveObject(
            model,
        ...
        TriggerEvent('inventory:removeItem', 'xmas_tree', 1)
        return false
    end
}
...
AddEventHandler('np-inventory:itemUsed', function(item, info, invName, itemSlot)
...
Citizen.CreateThread(function()
    ...
    exports["np-interact"]:AddPeekEntryByModel({ `prop_letterbox_01`, `prop_letterbox_02`, `prop_letterbox_03` }, {{
        ...
        isEnabled = function(pEntity)
            local info, mailType = getMailboxInfo(pEntity)
            return mailType == 'business' and (exports['np-business']:HasPermission(info, 'stash_access'))
    ...
    exports["np-interact"]:AddPeekEntryByModel({ `np_building_concrete_xl` }, {{
        event = "np-usableprops:pickupGlassBox",
        id = "np-usableprops:pickup_glassbox",
        icon = "arrow-up",
        label = "Pickup",
        parameters = {},
    }}, {
        distance = { radius = 5.0 },
        isEnabled = function(pEntity, pContext)
            local hasBox = exports['np-inventory']:hasEnoughOfItem('glassbox', 1, false, true)
          return hasBox and pContext.meta and pContext.meta.data.metadata and pContext.meta.data.metadata.isGlassBoxFloor
        end,
    })
...
AddEventHandler("np-usableprops:switchSign", function(args, entity, context)
    if context.model ~= 2037823845 and context.model ~= -986580443 then return end
...
AddEventHandler('np-objects:objectCreated', function(pObject, handle)
    ...
    if pObject.data.model == `v_corp_fib_glass1` then
        SetDisableFragDamage(handle, true)
    end
end)
...
AddEventHandler("np-usableprops:pickupGlassBox", function(args, entity, context)
    local box = exports['np-objects']:GetObjectByEntity(entity)
    if box then
        local success = exports['np-objects']:DeleteObject(box.id)
        for _, v in pairs(box.data.metadata.linkedObjects) do
            exports['np-objects']:DeleteObject(v)
        end
    end
end)
