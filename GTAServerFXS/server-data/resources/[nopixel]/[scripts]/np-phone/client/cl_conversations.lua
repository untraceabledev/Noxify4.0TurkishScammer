RegisterUICallback("np-ui:smsSend", function(data, cb)
  local number_from, number_to, source_number ,text_message = data.sender_number, data.number,data.source_number, data.message
...
RegisterUICallback("np-ui:getMessages", function(data, cb)
  ...
  local success, message
  if source_number ~= nil then
    success, message = RPC.execute("phone:getMessages", source_number, target_number, data.limit)
  else
    success, message = RPC.execute("phone:getMessages", number, target_number, data.limit)
  end
