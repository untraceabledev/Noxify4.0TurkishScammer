...
function SetSettings(settings)
    ...
    if Settings then
        ...
        -- SetPhoneVolume(Settings.phoneVolume)
        -- SetSubmixBalance('phone', Settings.phoneBalance)
        -- UpdateContextVolume("phone",  * 1.0)
    end
end
...
AddEventHandler("np:voice:transmission:state", function(serverID, context, transmitting, effect, isSourceInVehicle)
    ...
    if context == "radio" and isCivRadio and transmitting and not HasRangeFilterDisabled(CurrentChannel) then
...
RegisterNetEvent('np:voice:endpoint:override')
AddEventHandler('np:voice:endpoint:override', OverrideEndpoint)
