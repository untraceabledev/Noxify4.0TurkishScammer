...
local function GetInfo()
    ...
    if EndpointOverride then
        info[1], info[2] = EndpointOverride[1], EndpointOverride[2]
    end
...
function OverrideEndpoint(pEndpoint, enabled)
    if enabled then
        EndpointOverride = pEndpoint
    else
    ...
    Debug("[ENDPOINT] Override | Endpoint: %s | Enabled: %s", EndpointOverride or "N/A", _C(enabled, "enabled", "disabled"))
end

exports('OverrideEndpoint', OverrideEndpoint)
