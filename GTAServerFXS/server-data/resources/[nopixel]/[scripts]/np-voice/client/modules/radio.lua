RadioChannels, IsRadioOn, IsTalkingOnRadio, RadioVolume, CurrentChannel, TransmissionTime = {}, false, false, Config.settings.radioVolume, nil
...
function SetRadioFrequency(pFrequency, pSendAppEvent)
    if CanUseFrequency(pFrequency, true) then
        ...
        local success = RPC.execute("np-voice:addPlayerToRadio", pFrequency)
...
function StopTransmission(forced)
    ...
    RadioTimeout = TimeOut(300):next(function (continue)
        ...
        DegradeRadio(GetGameTimer() - TransmissionTime)
...
function SetRadioPowerState(state)
    if Throttled("radio:powerState") then return end
    IsRadioOn = state
...
function DegradeRadio(pTransmissionLength)
    if not CurrentChannel or CurrentChannel.id <= 20.0 then return end
    if pTransmissionLength < 1000 then return end

    local pct = pTransmissionLength / Config.radioDegradeDivider

    if not hasRadio() then
        return SetRadioPowerState(false)
    end

    TriggerServerEvent("np-voice:radioDegraded", pct)
end

function hasRadio()
    return exports["inventory"]:HasItem("radio", { quantity = 1, quality = 1 }) or exports["inventory"]:HasItem("civradio", { quantity = 1, quality = 1 })
end
