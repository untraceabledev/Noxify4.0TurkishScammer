...
FXAdderall = function(time)
  Citizen.CreateThread(function()
    StartScreenEffect("DeadlineNeon", 3.0, 0)
    local timer = GetCloudTimeAsInt() + time
    while GetCloudTimeAsInt() < timer do
      Wait(0)
    end
...
FXSteroids = function(time)
  Citizen.CreateThread(function()
    StartScreenEffect("BikerFormation", 5.0, 0)
    local timer = GetCloudTimeAsInt() + time
    while GetCloudTimeAsInt() < timer do
      Wait(0)
    end
...
InitCubes = function(setCount)
  Citizen.CreateThread(function()
    ...
    local counter = setCount or 4000
    local tick = 0
...
FXHeroin = function(time)
  Citizen.CreateThread(function()
    AnimpostfxPlay("DMT_flight", 1000, 1)
    InitCubes(1000)
    isHigh = true
    local step = 0
    local timer = GetCloudTimeAsInt() + time
