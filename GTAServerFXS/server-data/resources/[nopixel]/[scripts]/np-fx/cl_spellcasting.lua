...
AddEventHandler("fx:spellcast", function(Spell)
  if manaTotal > SpellList[Spell]["Base"]["Mana"] then
    ...
    exports['hud']:sendAppEvent({ mana = manaTotal })
...
Citizen.CreateThread(function()
  while true do
    ...
    if staffEquipped then
      ...
      exports["wounds"]:setEntityHealth(ped, GetEntityHealth(ped) + 5)
...
AddEventHandler("fx:staffused", function(equipped)
    ...
    if equipped then
      exports['wounds']:setArmor(0)
    end
    exports['hud']:sendAppEvent({ manaShow = equipped })
...
AddEventHandler("fx:spell:heal", function()
    local ped = PlayerPedId()
    exports["wounds"]:setEntityHealth(ped, GetEntityHealth(ped) + 50)
...
AddEventHandler("fx:spell:shock", function()
    ...
    exports["wounds"]:setEntityHealth(ped, GetEntityHealth(ped) - 33)
end)
