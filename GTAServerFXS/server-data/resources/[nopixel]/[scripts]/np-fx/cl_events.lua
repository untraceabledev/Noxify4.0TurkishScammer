...
function StartParticleOnEntity(ptDict, ptName, looped, entity, bone, offset, rot, scale, alpha, color, evolution, duration, pSpecialHandler)
    LoadParticleDictionary(ptDict)
    ...
    if pSpecialHandler and SpecialParticleHandlers[pSpecialHandler] then
        CreateThread(function()
            SpecialParticleHandlers[pSpecialHandler]()
        end)
    end
    ...
    if type(position.coords) == "table" then
        ...
        for _, coords in ipairs(position.coords) do
            ...
            Citizen.CreateThread(function()
                local particleHandle = StartParticleAtCoord(ptDict, ptName, looped, coords, position.rot, position.scale, position.alpha, position.color, duration or 0)
        ...
        local particleHandle = StartParticleAtCoord(ptDict, ptName, looped, position.coords, position.rot, position.scale, position.alpha, position.color, duration or 0)
...
AddEventHandler("particle:sync:entity", function(ptDict, ptName, looped, target, bone, position, duration, ptID)
    ...
    if type(bone) == "table" then
        ...
        for _, boneName in ipairs(bone) do
            ...
            Citizen.CreateThread(function()
                local particleHandle = StartParticleOnEntity(ptDict, ptName, looped, entity, boneName, position.offset, position.rot, position.scale, position.alpha, position.color, position.evolution, duration or 0)
                particle:resolve(particleHandle)
            end)
        ...
        if duration == 0 and ptID then
            ActiveParticles[ptID] = particles
        end
    else
        local particleHandle = StartParticleOnEntity(ptDict, ptName, looped, entity, bone, position.offset, position.rot, position.scale, position.alpha, position.color, position.evolution, duration or 0)
...
AddEventHandler("particle:sync:player", function(ptDict, ptName, looped, target, bone, position, duration, ptID, pSpecialHandler)
    ...
    if type(bone) == "table" then
        ...
        for _, boneName in ipairs(bone) do
            ...
            Citizen.CreateThread(function()
                local particleHandle = StartParticleOnEntity(ptDict, ptName, looped, entity, boneName, position.offset, position.rot, position.scale, position.alpha, position.color, position.evolution, duration or 0, pSpecialHandler)
                particle:resolve(particleHandle)
            end)
        ...
        local particleHandle = StartParticleOnEntity(ptDict, ptName, looped, entity, bone, position.offset, position.rot, position.scale, position.alpha, position.color, position.evolution, duration or 0, pSpecialHandler)
...
function StopParticle(pParticleHandle)
    if not pParticleHandle then return false end
    StopParticleFxLooped(pParticleHandle, 0)
end

-- Useful for local particles
exports("StartParticleAtCoord", StartParticleAtCoord)
exports("StopParticle", StopParticle)
