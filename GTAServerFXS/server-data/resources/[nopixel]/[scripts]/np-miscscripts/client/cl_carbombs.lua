...
AddEventHandler('np-inventory:itemUsed', function (name, info)
  ...
  local elements = {
    ...
    { name = "gridSize", label = "Grid Size (5-8)", icon = "time", _type = "number" },
    { name = "coloredSquares", label = "Colored Sqaures (5-10)", icon = "time", _type = "number" },
    { name = "timeToComplete", label = "Time To Complete (15-30 Seconds)", icon = "time", _type = "number" },
  }
  ...
  if ticksBeforeExplode < 10 then
    return TriggerEvent('DoLongHudText', 'Min ticks before explosion needs to be more than 10', 2)
  end
