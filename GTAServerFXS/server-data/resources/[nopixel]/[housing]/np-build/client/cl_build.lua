...
function Build.func.CleanUpArea()
    ...
    repeat
        ...
        local distance = #(plycoords - pos)
        if distance < 50.0 and ObjectFound ~= playerped then
        	if not NetworkGetEntityIsNetworked(ObjectFound) and not IsEntityAPed(ObjectFound) and not IsEntityAVehicle(ObjectFound) and not IsEntityAttached(ObjectFound) and not Entity(ObjectFound).state.DontClear then
