...
function GetEntityContext(pEntity, pEntityType, pEntityModel)
    ...
    if context.type == 1 then
        ...
        context.isDead = context.flags["isDead"]
...
function isCloseToLockedTrunk(pEntity, pPlayerPed, pDistance, pMustBeOpen)
    return #(getTrunkOffset(pEntity) - GetEntityCoords(pPlayerPed)) <= (pDistance or 1.0) and GetVehicleDoorLockStatus(pEntity) == 2 and (not pMustBeOpen or GetVehicleDoorAngleRatio(pEntity, 5) >= 0.1)
end
...
function isCloseToLockedHood(pEntity, pPlayerPed, pDistance, pMustBeOpen)
    return #(getFrontOffset(pEntity) - GetEntityCoords(pPlayerPed)) <= (pDistance or 1.0) and GetVehicleDoorLockStatus(pEntity) == 2 and (not pMustBeOpen or GetVehicleDoorAngleRatio(pEntity, 4) >= 0.1)
end
...
function isCloseToBoot(pEntity, pPlayerPed, pDistance, pModel)
    ...
    if skipCheck[model] then return true end
...
function hasDoor(pEntity, pDoor)
    local model = GetEntityModel(pEntity)

    if skipCheck[model] then return true end

    return DoesVehicleHaveDoor(pEntity, pDoor)
end
...
AddEventHandler("shops:vendingMachine", function (pParams, pEntity, pContext)
    if not pContext then return end
    local shopId = pContext.flags["isVendingMachineBeverage"] and "vending_drink" or "vending_food"
    exports["stores"]:open(shopId)
end);
