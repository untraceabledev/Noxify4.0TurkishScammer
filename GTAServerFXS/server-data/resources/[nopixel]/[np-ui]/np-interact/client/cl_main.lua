    ...
    local addEntry = function(pId, pEntry, pContext, pRelated)
        ...
        if metadata and options.meta then
            for k,v in pairs(options.meta) do
                local isCB = metadata[k] ~= nil and (type(v) == 'function' or (type(v) == "table" and v["__cfx_functionReference"] ~= nil))

                if isCB and v(metadata[k], metadata) ~= true then
                    return
                elseif not isCB and metadata[k] ~= v then
                    return
                end
            end
        end
    ...
    for id, options in pairs(pTracked) do
        ...
        callbacks[#callbacks + 1] = function(pDistance)
            ...
            if inRange and isEnabled and not visible then
                ...
                updateRequired = true
                entries[id] = true
                if options.highlight and entity then
                    SetEntityDrawOutline(entity, true)
                    highlightedEntities[entity] = true
                end
...
function StopPeekin()
    ...
    for entity,_ in pairs(highlightedEntities) do
