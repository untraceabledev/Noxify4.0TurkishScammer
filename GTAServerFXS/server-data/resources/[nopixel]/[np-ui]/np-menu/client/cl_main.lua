MenuTypes, MenuEntries, MenuItems = {'general', 'peds', 'vehicles', 'objects', 'news', 'k9', 'judge', 'games'}, {}, {}
...
function CheckSubMenuItems(pAction, pItems, pEntity, pContext)
    for index, item in ipairs(pItems) do
        local entry = MenuItems[item]
        if not entry then
            print("Menu item not found: " .. item)
            goto continue
        end
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if IsControlPressed(1, keybindControls[keyBind]) and GetLastInputMethod(2) and not locked then
            ...
            local isCuffed = exports["isPed"]:isPed("handcuffed")
            local recentlyCuffed = exports["isPed"]:isPed("recentcuff")
            if ((not isCuffed and not recentlyCuffed) or ((isPolice or isMedic) and isDead)) and (not locked or bypassed) then
                showMenu = true
...
RegisterNetEvent("temp:setBypassed", function()
    bypassed = true
end)
