...
setImmediate(async () => {
    while (!exports['config'].IsConfigReady()) {
        await Delay(100);
    }
    const spawnPublicZones = exports["config"].GetMiscConfig('crafting.spawn.public');
    ...
    craftingSpots.forEach(spot => {
        ...
        const serverCode = exports["config"].GetServerCode();
