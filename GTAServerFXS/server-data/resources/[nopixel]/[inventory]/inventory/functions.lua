...
function IsAllowedToCraft(pItemId)
    local characterId = exports["isPed"]:isPed("cid")

    local config = exports["config"]:GetModuleConfig("np-inventory:crafting:permissions")

    local allowlist = config.items[pItemId]
...
RegisterNetEvent('tranqed:enable')
AddEventHandler('tranqed:enable', function(sent)
    tranqed = sent
end)
...
AddEventHandler('RunUseItem', function(itemid, slot, inventoryName, isWeapon, passedItemInfo)
    ...
    if itemid == "butterfly_knife" then
        if tonumber(ItemInfo.quality) > 0 then
              local katanaInfo = json.decode(ItemInfo.information)
              katanaInfo.componentVariant = "1"
              local hiddenKeys = katanaInfo._hideKeys or {}
              hiddenKeys[#hiddenKeys + 1] = "componentVariant"
              katanaInfo._hideKeys = hiddenKeys
              TriggerEvent("equipWeaponID","-538741184",json.encode(katanaInfo),ItemInfo.id)
        end
        justUsed = false
        retardCounter = 0
        lastCounter = 0
    ...
    if itemid == "yokai_katana" then
        if tonumber(ItemInfo.quality) > 0 then
              ...
              hiddenKeys[#hiddenKeys + 1] = "componentVariant"
              katanaInfo._hideKeys = hiddenKeys
              TriggerEvent("equipWeaponID","3713923289",json.encode(katanaInfo),ItemInfo.id)
        end
        justUsed = false
        retardCounter = 0
        lastCounter = 0
    ...
    if (not IsPedInAnyVehicle(player)) then
        ...
        if (itemid == "np_prop_cs_walking_stick") then
            TriggerEvent('attach:np_prop_cs_walking_stick')
          end
    ...
    if (
      ...
      or itemid == "badlean"
        ...
        if success then
            ...
            if itemid == "moonshine" then alcoholStrength = 4.0 end
            if itemid == "lean" then
                alcoholStrength = 2.5
                TriggerEvent("Evidence:StateSet",2,1200)
                TriggerEvent("Evidence:StateSet",6,1200)
                TaskItem("amb@world_human_drinking@coffee@male@idle_a", "idle_c", 49, 0, "Lean!", "hadcrack", true,itemid,playerVeh)
                TriggerEvent( "player:receiveItem","emptystyrofoamcup",1)
            end
            if itemid == "badlean" then
                alcoholStrength = 4.0
                TriggerEvent("Evidence:StateSet",2,1200)
                TriggerEvent("Evidence:StateSet",6,1200)
    ...
    if (itemid == "coffee" or itemid == "frappuccino" or itemid == "latte" or itemid == "customcoffeeitem" or itemid == "ron_energy_drink") then
    ...
    if (itemid == "bananammo") then
        local finished = exports["np-taskbar"]:taskBar(5000,"Reloading",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
            TriggerEvent("actionbar:ammo",-494831054,20,true)
            remove = true
        end
    end
    ...
    if (itemid == "tranqammo") then
        local finished = exports["np-taskbar"]:taskBar(10000,"Reloading",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
            TriggerEvent("actionbar:ammo",358025388,2,true)
            remove = true
        end
    end

    if (itemid == "cannonammo") then
        local finished = exports["np-taskbar"]:taskBar(25000,"Reloading",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
            TriggerEvent("actionbar:ammo",1003267566,20,true)
            remove = true
        end
    end
    ...
    if (itemid == "gunpaintbucket") then
        if not (hasEnoughOfItem("refinedsteels", 3, false) and hasEnoughOfItem("refinedplastics", 3, false)) then
            ...
            local finished = exports["np-taskbar"]:taskBar(5000, "Filling Paint Gun", true, false)
    ...
    if (itemid == "metamorphine_box") then
        local finished = exports["np-taskbar"]:taskBar(10000,"Unpacking box",true,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
            remove = true
            TriggerEvent( "player:receiveItem", "Desomorphine", 20 )
        end
    end
    ...
    if (itemid == "ollyrancherbag") then
        TriggerEvent( "player:receiveItem","ollyrancher",3)
        TriggerEvent("inventory:DegenLastUsedItem",25)
    end

    if itemid == "coughsyrup" or itemid == "sprunk" then
        local mixItem, mixWith1, mixWith2
        if itemid == "coughsyrup" then
            mixItem = "lean"
            mixWith1 = "sprunk"
            mixWith2 = "ollyrancher"
        else
            mixItem = "lean"
            mixWith1 = "coughsyrup"
            mixWith2 = "ollyrancher"
        end
        if hasEnoughOfItem(mixWith1, 1, false) and hasEnoughOfItem(mixWith2, 1, false) then
            local finished = exports["np-taskbar"]:taskBar(10000, "Mixing", false, false, playerVeh)
            if finished == 100 and hasEnoughOfItem(itemid, 1, false) then
                remove = true
                TriggerEvent("inventory:removeItem", mixWith1, 1)
                TriggerEvent("inventory:removeItem", mixWith2, 1)
                if math.random() > 0.05 then
                    TriggerEvent("player:receiveItem", mixItem, 1)
                else
                    TriggerEvent("player:receiveItem", "bad" .. mixItem, 1)
                end
            end
        else
            TriggerEvent("DoLongHudText", "You need something to mix with.", 2)
        end
    end

    if (itemid == "ollyrancher") then
        TriggerEvent("animation:PlayAnimation","pill")
        local finished = exports["np-taskbar"]:taskBar(3000,"Eating Olly Rancher.",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
            remove = true
        end
    end
    ...
    if (itemid == "glass") then
        ...
        local whitelistBong = IsAllowedToCraft("weedbong")

        if whitelistBong then
    ...
    if (itemid == "weightlosspill") then
        TriggerEvent("animation:PlayAnimation","pill")
        local finished = exports["np-taskbar"]:taskBar(3000,"Losing weight! 💊",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
            remove = true
            TriggerEvent("animation:PlayAnimation","cancel")
            Citizen.Wait(100)
    ...
    if (itemid == "adderall_pill") then
        TriggerEvent("animation:PlayAnimation","pill")
        local finished = exports["np-taskbar"]:taskBar(3000,"Gotta focus!",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
            local drugEffect = true
    ...
    if (itemid == "test_pill") then
        TriggerEvent("animation:PlayAnimation","pill")
        local finished = exports["np-taskbar"]:taskBar(3000,"Taking testosterone.",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
            remove = true
        end
    end
        ...
        if itemInfo and itemInfo.quality > 0 then
            ...
            if currentUses == 0 then
                ...
                if (finished == 100 and hasEnoughOfItem(itemid, 1, false)) then
                    ...
                    TriggerEvent("Evidence:StateSet",2,1200)
                    TriggerEvent("Evidence:StateSet",6,1200)
                    NPX.Events.emitNet('np-fx:sound:playSoundFromSelf', 'SNIFFA', 'DLC_NIKEZ_GENERAL', 'GENERAL_GENERAL');
                    ...
                    if isCocainium then
                        TriggerServerEvent("np-drugeffects:drugsTaken", cid, "trace radiation")
                    end
                end
            end
        end
    ...
    if string.match(itemid, "^experiment_%d+$") then
        TriggerEvent("animation:PlayAnimation", "pill")
    ...
    if itemid == "pillbox" then
        ...
        if finished == 100 and hasEnoughOfItem(itemid, 1, false) then
            for i = 1, 9 do
                TriggerEvent("player:receiveItem", "experiment_" .. string.format("%02d", i), 5)
            end
        end
    end
    ...
    if itemid == "tablet" then
        TriggerEvent("laptop:open")
    end

    if itemid == "dashcam" then
        TriggerEvent("gopixel:useDashCam")
    end

    if itemid == "notebook" then
        TriggerEvent("notebook:open")
    end
...
function GetInfoForFirstItemOfName(item_id)
    local info = nil
    for i,v in pairs(clientInventory) do
        if v.item_id == item_id and (info == nil or v.slot < info.slot) then
            info = {
...
AddEventHandler('np-business:repairItem', function()
    ...
    local itemListInfo = json.decode(exports["inventory"]:itemListInfo(info.item_id))
    local allowed = true
...
function getInformationFromFirstItem(itemToFind)
  local foundItem = exports["inventory"]:GetInfoForFirstItemOfName(itemToFind)
...
AddEventHandler("cokeline:insert", function(pOriginInventory, pTargetInventory, pOriginSlot, pTargetSlot, pOriginItemId, pTargetItemId, pOriginItemInfo, pTargetItemInfo)
    ...
    local finished = exports["np-taskbar"]:taskBar(15000, "Filling with " .. cConfig.fillString, false, false)
    if finished ~= 100 then return end
...
AddEventHandler("np-spawn:characterSpawned", function()
    local amount = exports["inventory"]:getQuantity('571920712')
    TriggerServerEvent("inventory:checkMoney", amount)
end)

RegisterNetEvent("inventory:checkMoney")
AddEventHandler("inventory:checkMoney", function()
    local amount = exports["inventory"]:getQuantity('571920712')
    TriggerServerEvent("inventory:checkMoney", amount)
end)

exports("SetInventoryDisabled", function(pState)
    inventoryDisabled = pState
end)

exports("OpenInventory", function(pData)
    TriggerEvent("server-inventory-open", "1", pData[1])
end)

exports("HasItem", function(pData)
    return hasEnoughOfItem(pData,1,false)
end)

exports("UseBodySlot", function(pData)
    return hasEnoughOfItem(pData,1,false)
end)
...
exports("GetItemList", function(pData)
    return itemList
end)

exports("SetActionBarDisabled", function(pData)
    return true
end)

RegisterCommand('giveitem', function(src, pArgs)
    TriggerEvent("player:receiveItem", pArgs[1], pArgs[2])
end)

RegisterCommand('updateData', function(src, pArgs)
    local data = {
        voziloinf2 = "N/A",
        brojinf2 = "505-332-540",
        imeinf = "Jigger TSX",
        imeinf2 = "Citizen ID: 750",
        kucainf2 = "Little Seoul Apartments - C145"
    }
