...
let WhitelistInventoriji = {
    'vesmasina-': ['rollcash'],
    'weedrack-': ['wetbud'],
    'grime-': ['dodopackagesmall', 'dodopackagemedium', 'dodopackagelarge'],
    'gruppe6pickup-': [],
    'gruppe6storage-': ['gruppe6bag'],
    'gruppe6davidstorage-': ['gruppe6bag'],
    'GoPostal-': ['dodopackagesmall', 'dodopackagemedium', 'dodopackagelarge'],
    'JunkieStorage-': [],
    'Pawnshop-': ['Hobonickel1', 'stolen2ctchain', 'stolen5ctchain', 'stolen8ctchain', 'stolen10ctchain', 'stolengameboy', 'stolencasiowatch', 'stoleniphone', 'stolennokia', 'stolenoakleys', 'stolenpixel3', 'stolenpsp', 'stolenraybans', 'stolens8', 'rolexwatch', 'mugbeer', 'Heirloom', 'stolentv', 'stolenmicrowave', 'stolenmusic', 'jewelry_part', 'gallerygem', 'stolenart', 'stolencomputer', 'goldbar', 'goldcoin', 'negg', 'pegg', 'silvercoin', 'gruppe6bag', 'carhood', 'gepek', 'np_doors', 'np_tire'],
};
...
const foodCatMaps = {
    dairy: 'Mlecni proizvodi vam pomazu da se osecate smirenije.',
    grain: 'Zitarice te zaista ispunjavaju!',
    oil: 'Ulja zaista pomazu kod vaseg nivoa stresa.',
    leavening: 'Ostavljanje pomaze vasoj intelekciji.',
    protein: 'Proteini vam pomazu da se osecate snaznije.',
    seasoning: 'Zacin cini da vas oni oko vas vise cene!',
    sugar: 'Secer pomaze u fokusiranju.',
    vegetables: 'Povrce zaista poboljsava vas kardio.',
}
...
dozvolatreba['453432689'] = true;
dozvolatreba['1593441988'] = true;
dozvolatreba['-1075685676'] = true;

craftoruzija['-1074790547'] = true;
craftoruzija['497969164'] = true;
craftoruzija['-2084633992'] = true;
craftoruzija['487013001'] = true;
craftoruzija['-275439685'] = true;
craftoruzija['1649403952'] = true;
craftoruzija['615608432'] = true;
craftoruzija['-134995899'] = true;
craftoruzija['-942620673'] = true;
craftoruzija['-1472189665'] = true;
craftoruzija['2460120199'] = true;
craftoruzija['148457251'] = true;
craftoruzija['-2012211169'] = true;
craftoruzija['3638508604'] = true;
craftoruzija['1923739240'] = true;


$(document).ready(function () {

    function postaviProzirnost(klasa, prozirnost) {
        var elementi = document.getElementsByClassName(klasa);

        for (var i = 0; i < elementi.length; i++) {
            elementi[i].style.opacity = prozirnost;
        }
    }
    ...
    $('.save-settings').click(() => {
        ...
        $.post(
            'https://inventory/UpdateSettings',
            JSON.stringify({
    ...
    window.addEventListener('message', function (event) {
        ...
        if (item.response == 'openGui') {
            ...
            $('#app').fadeIn();
            $('#containers-wrapper').css({ display: "grid" }).slideDown(400).style;
            ...
            broughtJoint = item.broughtJoint;
            brought = item.brought;
            isCop = item.cop;
            skelly = item.skelly
            charName = item.charName
            charPhoneNumber = item.charPhoneNumber
            charHome = item.charHome
            charCid = item.charCid
            postaviProzirnost('inventory-glava', (skelly.wounds.head.damage / 10));
            postaviProzirnost('inventory-trup', (skelly.wounds.body.damage / 10));
            postaviProzirnost('inventory-leftruka', (skelly.wounds.leftArm.damage / 10));
            postaviProzirnost('inventory-rightruka', (skelly.wounds.rightArm.damage / 10));
            postaviProzirnost('inventory-leftnoga', (skelly.wounds.leftLeg.damage / 10));
            postaviProzirnost('inventory-rightnoga', (skelly.wounds.rightLeg.damage / 10));

            $(".headbarinv2").stop().animate({
                'width': (100 - ((skelly.wounds.head.damage * 10))) + "%"
            }, 1);
            $(".trupbarinv2").stop().animate({
                'width': (100 - ((skelly.wounds.body.damage * 10))) + "%"
            }, 1);
            $(".levarukainv2").stop().animate({
                'width': (100 - ((skelly.wounds.leftArm.damage * 10))) + "%"
            }, 1);
            $(".desnarukainv2").stop().animate({
                'width': (100 - ((skelly.wounds.rightArm.damage * 10))) + "%"
            }, 1);
            $(".levanogainv2").stop().animate({
                'width': (100 - ((skelly.wounds.leftLeg.damage * 10))) + "%"
            }, 1);
            $(".desnanogainv2").stop().animate({
                'width': (100 - ((skelly.wounds.rightLeg.damage * 10))) + "%"
            }, 1);
...
function UpdateQuality(data, penis) {
    ...
    let weight = Number(item.dataset.weight);
    ...
    let htmlstring =
        ...
        "<span class='item-qty'>" + itemcountc + "x</span> <span class='item-weight'>" + weight.toFixed(2) + "</span> </div>"
        +
        `<img src='${image}` +
        "' data-info='" +
        escape(info) +
...
function UseBar(itemid, text, amount) {
    ...
    $(newElement).css('left', '-30vw')
    $('#UseBar').css({ display: "flex" }).fadeIn(1000);
    $(newElement).animate({ left: '0vw' }, 1000)
    setTimeout(() => {
        $(newElement).animate({ left: '-30vw' }, 500).fadeOut(2000)
    }, 3500);
...
function ToggleBar(toggle, boundItems, boundItemsAmmo) {
    if (toggle) {
        ...
        if (boundItems[1]) {
            ...
            htmlstring =
                "<div class='item3' ><div class='quickbind' id='bind1'> 1 </div><div class='itemname3'>  </div> <img src='icons/empty.png' class='itemimage'>  </div>";
        }
        ...
        for (let i = 2; i < 6; i++) {
            if (boundItems[i]) {
                ...
                htmlstring =
                    htmlstring +
                    "<div class='item3' >" + "<div class='quickbind' id='bind" + i + "'>" + "i" + "</div> " + "<div class='itemname3'>  </div> <img src='icons/empty.png' class='itemimage'>  </div>";
            }
        }
...
function invStack(
    ...
    $.post('https://inventory/stack', JSON.stringify(arr));
}
...
function invMove(
    ...
    $.post('https://inventory/move', JSON.stringify(arr));
}
...
function invSwap(targetSlot, targetInventory, originSlot, originInventory, itemid1, metainformation1, itemid2, metainformation2) {
    ...
    $.post('https://inventory/swap', JSON.stringify(arr));
}

function removeCraftItems(itemid, moveAmount, CraftArrayID) {
    console.log(CraftArrayID)
    let arr = itemList[itemid].craft[CraftArrayID];
    let amount = moveAmount;
    $.post('https://inventory/removeCraftItems', JSON.stringify([arr, amount]));
}
...
function CreateEmptyPersonalSlot(slotLimit) {
    ...
    let htmlstring = '';
    let bindSlotString = '';
    let extraSlotString = ''; // Yeni slotların eklenmesi gereken yer
    for (i = 1; i < slotLimit + 1; i++) {
        if (i <= 20) {
            if (bindSlots.includes(i)) {
                bindSlotString += "<div id='playerslot" + i + "' class='item'><div id='bind" + i + "'>" + i + "</div></div>";
            } else {
                htmlstring += "<div id='playerslot" + i + "' class='item'></div>";
            }
        } else {
            if (bindSlots.includes(i)) {
                bindSlotString += "<div id='playerslot" + i + "' class='item'><div id='bind" + i + "'>" + i + "</div></div>";
            } else {
                extraSlotString += "<div id='playerslot" + i + "' class='item'></div>";
            }
        }
    }
    document.getElementById('hotbar').innerHTML = bindSlotString;
    document.getElementById('playerslots').innerHTML += htmlstring;
    document.getElementById('inventory-extra').innerHTML = extraSlotString; // Ekstra slotları eklemek için
}
...
function DisplayInventoryMultiple(playerinventory, itemCount, invName, targetinventory, targetitemCount, targetinvName, cash, Owner, targetInvWeight, targetInvSlots, display) {
    // console.log(dropinventory, dropitemCount, dropinvName)
    secondaryWeight = 0;
    StoreOwner = Owner;
    PlayerStore = false;
    ...
    if (targetinvName.indexOf('Drop') > -1) {
        ...
        slotLimitTarget = 200;
    } else if (targetinvName.indexOf('warehouse2') > -1) {
        ...
        slotLimitTarget = 90;
    } else if (targetinvName.indexOf('motel4') > -1) {
        secondaryMaxWeight = 1000.0;
        slotLimitTarget = 90;
    } else if (targetinvName.indexOf('honestwinebox') > -1) {
        secondaryMaxWeight = 30.0;
        slotLimitTarget = 6;
        displayName = 'Kutija Vina';
    } else if (targetinvName.indexOf('6STRTunerShop') > -1) {
        secondaryMaxWeight = 2000.0;
        slotLimitTarget = 200;
    } else if (targetinvName.indexOf('zlatara-repair-inv') > -1) {
        secondaryMaxWeight = 30.0;
        slotLimitTarget = 1;
        displayName = 'Nakit Popravka';
    } else if (targetinvName.indexOf('inspekcija-zlatara') > -1) {
        secondaryMaxWeight = 30.0;
        slotLimitTarget = 1;
        displayName = 'Nakit Inspekcija';
    } else if (targetinvName.indexOf('roostertakeout') > -1) {
        secondaryMaxWeight = 5.0;
        slotLimitTarget = 5;
    } else if (targetinvName.indexOf('roosterstash') > -1) {
        secondaryMaxWeight = 2000.0;
        slotLimitTarget = 400;
        displayName = 'Roosters Rest Frizider';
    } else if (targetinvName.indexOf('midnightstash') > -1) {
        secondaryMaxWeight = 1000.0;
        slotLimitTarget = 400;
        displayName = 'Midnight Stash';
    } else if (targetinvName.indexOf('gsfkucastash') > -1) {
        secondaryMaxWeight = 2000.0;
        slotLimitTarget = 400;
        displayName = 'Midnight Stash';
    } else if (targetinvName.indexOf('frizider') > -1) {
        secondaryMaxWeight = 2000.0;
        slotLimitTarget = 200;
        displayName = 'frizider';
    } else if (targetinvName.indexOf('RoostersStashPrivate') > -1) {
        secondaryMaxWeight = 2000.0;
        slotLimitTarget = 400;
        displayName = 'Roosters Rest Sef';
    } else if (targetinvName.indexOf('BurgerShotPolica') > -1) {
        secondaryMaxWeight = 200.0;
        slotLimitTarget = 10;
    } else if (targetinvName.indexOf('container-jewelry') > -1) {
        secondaryMaxWeight = 30.0;
        slotLimitTarget = 5;
        displayName = 'Nakit';
    } else if (targetinvName.indexOf('container-protected') > -1) {
        secondaryMaxWeight = 70.0;
        slotLimitTarget = 5;
        displayName = 'Protected Slots';
    } else if (targetinvName.indexOf('casinostashboss') > -1) {
        secondaryMaxWeight = 2000.0;
        slotLimitTarget = 200;
    } else if (targetinvName.indexOf('casinostash') > -1) {
        secondaryMaxWeight = 1000.0;
        slotLimitTarget = 200;
    ...
    } else if (targetinvName.indexOf('tacokesa') > -1) {
        secondaryMaxWeight = 5.0;
        slotLimitTarget = 5;
        displayName = 'Taco Kesa';
    } else if (targetinvName.indexOf('hornyskesa') > -1) {
        secondaryMaxWeight = 6.0;
        slotLimitTarget = 6;
        displayName = 'Hornys Kesa';
    } else if (targetinvName.indexOf('kesabato') > -1) {
        secondaryMaxWeight = 10.0;
        slotLimitTarget = 1;
        displayName = 'Kesa';
    } else if (targetinvName.indexOf('wrappedgift') > -1) {
        secondaryMaxWeight = 10.0;
        slotLimitTarget = 1;
        displayName = 'Poklon';
    } else if (targetinvName.indexOf('collectiblepouch') > -1) {
        secondaryMaxWeight = 10.0;
        slotLimitTarget = 3;
        displayName = 'Vreca';
    } else if (targetinvName.indexOf('seedbag') > -1) {
        secondaryMaxWeight = 30.0;
        slotLimitTarget = 3;
        displayName = 'Kesica za sjemenke';
    } else if (targetinvName.indexOf('pizzabox') > -1) {
        secondaryMaxWeight = 5.0;
        slotLimitTarget = 5;
        displayName = 'Pizza Box';
    } else if (targetinvName.indexOf('bentobox') > -1) {
        secondaryMaxWeight = 5.0;
        slotLimitTarget = 5;
        displayName = 'Bento Box';
    } else if (targetinvName.indexOf('tcgbinder') > -1) {
        secondaryMaxWeight = 0.0;
        slotLimitTarget = 56;
        displayName = 'Card Binder';
    } else if (targetinvName.indexOf('OttosStash') > -1) {
        secondaryMaxWeight = 2000.0;
        slotLimitTarget = 200;
    } else if (targetinvName.indexOf('bennysstash') > -1) {
        secondaryMaxWeight = 2000.0;
        slotLimitTarget = 200;
    } else if (targetinvName.indexOf('smiljastash') > -1) {
        secondaryMaxWeight = 2000.0;
        slotLimitTarget = 200;
    } else if (targetinvName.indexOf('GalerijaProcenaGema') > -1) {
        secondaryMaxWeight = 1.0;
        slotLimitTarget = 1;
    } else if (targetinvName.indexOf('6STRTunerShopPrivate') > -1) {
        secondaryMaxWeight = 2000.0;
        slotLimitTarget = 400;
    } else if (targetinvName.indexOf('Gem_Trade') > -1) {
        secondaryMaxWeight = 2000.0;
        slotLimitTarget = 200;
    } else if (targetinvName.indexOf('galerija2') > -1) {
        secondaryMaxWeight = 2000.0;
        slotLimitTarget = 200;
    } else if (targetinvName.indexOf('SefWarehouse') > -1) {
        secondaryMaxWeight = 2000.0;
        slotLimitTarget = 200;
    } else if (targetinvName.indexOf('Organizacija') > -1) {
        secondaryMaxWeight = 2000.0;
        slotLimitTarget = 200;
        ...
        secondaryMaxWeight = 2000.0;
        slotLimitTarget = 400;
        displayName = 'Roosters Rest Stash';
    } else if (targetinvName.indexOf('RoostersStashPrivate') > -1) {
        secondaryMaxWeight = 2000.0;
        slotLimitTarget = 400;
        displayName = 'Roosters Rest Private';
    } else if (targetinvName.indexOf('police') > -1) {
        ...
        secondaryMaxWeight = 2000.0;
        slotLimitTarget = 40;
    } else if (targetinvName.indexOf("hornys") > -1) {
        secondaryMaxWeight = 100.0
        slotLimitTarget = 10;
        displayName = 'Hornys';
    } else if (targetinvName.indexOf("bennys") > -1) {
        secondaryMaxWeight = 100.0
        slotLimitTarget = 10;
        displayName = 'Bennys';
    } else if (targetinvName.indexOf("tacopult1") > -1) {
        secondaryMaxWeight = 100.0
        slotLimitTarget = 10;
        displayName = 'Taco Pult';
    } else if (targetinvName.indexOf("tacopult2") > -1) {
        secondaryMaxWeight = 100.0
        slotLimitTarget = 10;
        displayName = 'Taco Pult';
    } else if (targetinvName.indexOf("burgershot") > -1) {
        secondaryMaxWeight = 100.0
        slotLimitTarget = 10;
        displayName = 'Burgershot';
    } else if (targetinvName.indexOf("burgershot2") > -1) {
        secondaryMaxWeight = 100.0
        slotLimitTarget = 10;
        displayName = 'Burgershot';
    } else if (targetinvName.indexOf('xmasgiftcoal') > -1) {
        secondaryMaxWeight = 50.0;
        slotLimitTarget = 5;
        displayName = 'XMas';
    } else if (targetinvName.indexOf("burgershot3") > -1) {
        secondaryMaxWeight = 100.0
        slotLimitTarget = 10;
        displayName = 'Burgershot';
    } else if (targetinvName.indexOf("jewled_dragon") > -1) {
        secondaryMaxWeight = 100.0
        slotLimitTarget = 5;
        displayName = 'Jewled Dragon';
    } else if (targetinvName.indexOf("hayes_pult") > -1) {
        secondaryMaxWeight = 35.0
        slotLimitTarget = 5;
        displayName = 'Hayes Pult';
    } else if (targetinvName.indexOf("jewled_dragon") > -1) {
        secondaryMaxWeight = 100.0
        slotLimitTarget = 5;
        displayName = 'Jewled Dragon';
    } else if (targetinvName.indexOf("storage-BurgerShot") > -1) {
        secondaryMaxWeight = 4000.0
        slotLimitTarget = 200;
        displayName = 'Burger Shot Storage';
    } else if (targetinvName.indexOf("storage-hornys") > -1) {
        secondaryMaxWeight = 4000.0
        slotLimitTarget = 200;
        displayName = 'Hornys Storage';
    } else if (targetinvName.indexOf('ply') > -1) {
        secondaryMaxWeight = 400.0;
        slotLimitTarget = 35;
        displayName = 'Druga Osoba';
...
function BuildDrop(brokenSlots) {
    $.post(
        'https://inventory/dropIncorrectItems',
        JSON.stringify({
            slots: brokenSlots,
        }),
    );
        ...
        if (isEmpty(inventory[i])) {
            ...
            if (itemList[itemid] !== undefined) {
                ...
                if (itemList[itemid].foodCategory) {
                    meta = `${meta}<br><br>Kategorija hrane: ${[...itemList[itemid].foodCategory].map(f => capitalizeFirstLetter(f)).join(', ')}`;
                ...
                $.post("https://inventory/updateMyQuality", JSON.stringify({
                    item_id: itemid,
                    slot: slot,
                    quality: quality,
                    imeinv: inventoryName,
                }));
                ...
                let htmlstring =
                    ...
                    '' +   // name
                    " </div> <div class='information'>  " +
                    "<span class='item-qty'>" + itemcountc + "</span> <span class='item-weight'>" + weight.toFixed(2) + "</span></div>"
                    + `<img src='${image}` +
                    "' data-inventory='" +
                    inventoryNumber +
                    "' data-quality='" +
                ...
                if (TargetInventoryName.indexOf('Craft') > -1 && !main) {
                    ...
                    let fill =
                        ...
                        name +
                        " </div> <div class='information'> " +
                        "<span class='item-qty'>" + itemcountc + "</span> <span class='item-weight'>" + weight.toFixed(2) + "</span>"
                        + `</div> <img src='${image}` +
                        "' data-inventory='" +
                        inventoryNumber +
                        "' data-creationDate='" +
...
function UpdateSetWeights(secondaryName) {
    ...
    if (!$('#mainInvHeading').html() || $('#secondaryInvName').attr('title') !== TargetInventoryName) {
        let mainHeader = `
            ...
            <h1 id='mainInvName' class='inventory-nameBackPack' title='${PlayerInventoryName}'>Backpack</h1>
            <h1 id='info' class="inventory-name4">Personal Information</h1>
            <h1 id='infotelo' class="inventory-name3">Body</h1>
            <div class="inventory-ikonica">&nbsp;</div>
            <div class="inventory-ikonica2">&nbsp;</div>
            <div class="inventory-ikonica3">&nbsp;</div>
            <div class="inventory-backpackicon">&nbsp;</div>
            <div class="inventory-skeletonmrtvi">&nbsp;</div>
            <div class="inventory-skeletonmrtvi2">&nbsp;</div>

            <div class="inventory-glava">&nbsp;</div>
            <div class="inventory-trup">&nbsp;</div>
            <div class="inventory-leftruka">&nbsp;</div>
            <div class="inventory-rightruka">&nbsp;</div>
            <div class="inventory-leftnoga">&nbsp;</div>
            <div class="inventory-rightnoga">&nbsp;</div>

            <div class="headbarinv"  id="headbar">
            <div class="headbarinv2"  id="headbar2"></div> &nbsp;
           </div>

           <div class="trupbarinv"  id="trupbar">
           <div class="trupbarinv2"  id="trupbar2"></div> &nbsp;
          </div>

          <div class="levarukainv"  id="levarukainvbar">
          <div class="levarukainv2"  id="levarukainvbar2"></div> &nbsp;
         </div>
         
         <div class="desnarukainv"  id="desnarukainvbar">
         <div class="desnarukainv2"  id="desnarukainvbar2"></div> &nbsp;
        </div>

        <div class="levanogainv"  id="levanogainvbar">
        <div class="levanogainv2"  id="levanogainvbar2"></div> &nbsp;
       </div>

       <div class="desnanogainv"  id="desnanogainvbar">
       <div class="desnanogainv2"  id="desnanogainvbar2"></div> &nbsp;
      </div>

      <div class="voziloinf">Personal Vehicle</div>
      <div class="voziloinf2">N/A</div>
      <div class="brojinf">Phone Number</div>
      <div class="brojinf2">${charPhoneNumber}</div>
      <div class="imeinf">${charName}</div>
      <div class="imeinf2">Citizen ID: ${charCid}</div>
      <div class="kucainf">Home Location</div>
      <div class="kucainf2">${charHome}</div>



            <div class="inventory-weight"><span id='playerWeight'>${personalWeight.toFixed(2)}</span>kg / ${personalMaxWeight.toFixed(2)}</span>kg</div>
        ...
        let secondaryHeader = `<h1 id='secondaryInvName' class='inventory-name2' title='${TargetInventoryName}'>${secondaryName}</h1>`;

        if (!shop && !craft) {
            secondaryHeader += `
                <div class="inventory-weight2"><span id='secondaryWeight'>${secondaryWeight.toFixed(2)}</span>kg / ${secondaryMaxWeight.toFixed(2)}</span>kg</div>
...
$(document).on('mousedown mouseup', (event) => {
    ...
    if (blockedElements.includes(element.id)) return;
    ...
    if (event.button === 1) {
        ...
        // Orta fare düğmesi (tıklama olayı)
        if (element.id === 'DisplayInformation') {
            // DisplayInformation elementi ise
            // Toggle DisplayInformation görünürlüğünü
            if (isDisplayInformationOpen()) {
                closeDisplayInformation();
            } else {
                openDisplayInformation();
            }
        }
    } else {
...
function isDisplayInformationOpen() {
    // Burada DisplayInformation'ın açık olup olmadığını kontrol etmeniz gerekiyor
    // Örneğin:
    // return $('#DisplayInformation').css('display') !== 'none';
}

function openDisplayInformation() {
    // Burada DisplayInformation'ı açmanız gerekiyor
}

function closeDisplayInformation() {
    // Burada DisplayInformation'ı kapatmanız gerekiyor
}
...
function DragToggle(fromSlot, isUsing, mouseEvent) {
    ...
    if (fromSlot) {
        ...
        if (occupiedslot && !isDragging) {
            ...
            // $.post('https://inventory/SlotInuse', JSON.stringify(parseInt(draggingid.replace(/\D/g, ''))));
            ...
            if (moveAmount != 0 && $('#draggedItem > img')) {
                ...
                $('#draggedItem > .information').html(actualAmount + ' (' + $('#draggedItem > img').data('weight') + ')');
            }
...
function FindNextSlotAndMove(half) {
    ...
    let startingSlot = 6;
    let slotCounter = 35;
    if (clickedInventory == 'wrapmain' || clickedInventory == 'hotbar') {
...
function UpdateTextInformation(slot) {
    if (document.getElementById(slot)) {
        ...
        if (item) {
            ...
            informationDiv.html("<span class='item-qty'>" + itemcountc + "</span> <span class='item-weight'>" + weight.toFixed(2) + "</span>");
...
function ErrorMove() {
    // $.post('https://inventory/move:fail', JSON.stringify({}));
}

function SuccessMove() {
    // $.post('https://inventory/move:success', JSON.stringify({}));
}
...
function CompileStacks(
    ...
    var isWeapon = false;
    if (itemList[itemidsent].weapon != null && !exluded[itemidsent] && exluded[itemidsent] && !isCop && targetInventory !== originInventory) {
        brought = true;
...
function SwapStacks(targetSlot, originSlot, inv1, inv2, itemid1, metainformation1, itemid2, metainformation2) {
    // $.post('https://inventory/swapstack', JSON.stringify({
    //   slot1: slot1,
    //   slot2: slot2,
    //   inv1: inv1,
    //   inv2: inv2,
...
function closeInv(pIsItemUsed = false) {
    ...
    $.post(
        'https://inventory/ServerCloseInventory',
        JSON.stringify({
...
function CheckCraftFail(itemid, moveAmount) {
    ...
    for (let ArrayX = 0; ArrayX < requirementsArray.length; ArrayX++) {
        ...
        if (!itemcheck && !weightcheck) {
            ...
            $.post('https://inventory/craftProgression', JSON.stringify({
...
function AttemptDropInFilledSlot(slot) {
    ...
    let weightReturnItem = Number(itemReturnItem.dataset.weight);
    ...
    if (inventoryDropName === "hotbar") {
        inventoryDropName = "wrapmain";
    }
    if (inventoryReturnItemDropName === "hotbar") {
        inventoryReturnItemDropName = "wrapmain";
    }
    ...
    let prefix = TargetInventoryName.substring(0, TargetInventoryName.indexOf('-') + 1);

    if (WhitelistInventoriji[prefix] && inventoryDropName == 'wrapsecondary') { /////////// GAS

        var can = false;

        for (var k in WhitelistInventoriji[prefix]) {
            if (itemidsent == WhitelistInventoriji[prefix][k]) { can = true };
        }

        if (!can) {
            result = "Taj item neide tu.";
        }
    }
    ...
    if ((TargetInventoryName.startsWith("container") || TargetInventoryName.startsWith("fisher-bucket")) && (
        ...
        for (let i = 0; i < parseInt(MyItemCount); i++) {
            ...
            if (meta.inventoryId !== undefined && !TargetInventoryName.startsWith('container-protected')) {
    ...
    if (result == 'Success' && result2 == 'Success') {
        ...
        if (stacking) {
            ...
            if (TargetInventoryName == 'Shop' || (!StoreOwner && PlayerStore)) {
                ...
                if (itemidsent === "joint") {
                    moveAmount = 10
                    purchaseCost = parseInt(item.dataset.fwewef) * parseInt(moveAmount);
                }
                if (purchaseCost > userCash && purchaseCost !== 0) {
                    ...
                    if (itemList[itemidsent].weapon && inventoryReturnItemDropName !== inventoryDropName) {
                        ...
                        if (dozvolatreba[itemidsent] && !userWeaponLicense) {
                            result = 'Nemas dozvolu za oruzije!';
                            InventoryLog('Greska: ' + result);
                            EndDragError(slot);
                            return;
                        }

                        if (dozvolatreba[itemidsent] && brought && !isCop) {
                            result = 'Mozes kupit jednu pusku u danu!';
                            InventoryLog('Greska: ' + result);
                            EndDragError(slot);
                            return;
                        }
                    ...
                    if (itemidsent === "joint") {
                        if (itemidsent === "joint") {
                            if (broughtJoint) {
                                result = 'Mozes samo jednom kupit jointove tokom dana!';
                                InventoryLog('Greska: ' + result);
                                EndDragError(slot);
                                return;
                            }
                    ...
                    if (currentInventory == 2 && inventoryDropName == 'wrapmain') {
                        ...
                        purchase = true;
                        if (itemidsent === "joint") {
                            $.post("https://inventory/kupiojoint", JSON.stringify());
                        }
                        if (itemidsent === "1593441988" || itemidsent === "453432689" || itemidsent === "-1075685676") {
                            $.post("https://inventory/kupiogun", JSON.stringify());
                        }
                    }
                }
            }
...
function DisplayInformation(slot) {
    ...
    if (quality != '' && quality != null && quality != 'undefined') {
        element.html(
            '<h2 class="NameHeader">' + name + '</h2>' +
            '<div class="DispInfo">' + metainformation + '</div>' +
            '<span class="InfoItem"><span class="InfoValue">🔧 ' + weight.toFixed(2) + '</span><i class="fas fa-bag"></i></span>' +
            '<span class="InfoItem"><span class="InfoValue">💰 ' + amount + '</span><i class="fas fa-hammer"></i></span>' +
            '</div>'
        );
    } else {
        element.html(
    ...
    if ((TargetInventoryName.startsWith("container") || TargetInventoryName.startsWith("fisher-bucket")) && (inventoryReturnItemDropName == 'wrapmain' && inventoryDropName == 'wrapsecondary')) {
        ...
        if (TargetInventoryName.split("-").length > 3) {
            ...
            if (!hasWhitelistedKey) {
                result = "Ne mozes to staviti item ovde!";
            }
        }
    }
        ...
        if (TargetInventoryName == 'Shop' || (!StoreOwner && PlayerStore)) {
            ...
            if (purchaseCost > userCash && purchaseCost !== 0) {
                ...
                if (currentInventory == 2 && inventoryDropName == 'wrapmain') {
                    ...
                    if (itemidsent === "1593441988" || itemidsent === "453432689" || itemidsent === "-1075685676") {
                        $.post("https://inventory/kupiogun", JSON.stringify(true));
                    }
                }
            }
        }
...
function giveItemClosest() {
    RequestItemData();
    let isWeapon = itemList[itemid].weapon;
    let moveAmountGive = parseInt(document.getElementById('move-amount').value);
    if (isWeapon === undefined) {
        isWeapon = false;
    }
    if (currentInventory == 1) {
        inventoryUsedName = PlayerInventoryName;
        let arr = [inventoryUsedName, itemid, slotusing, isWeapon, itemusinginfo, moveAmountGive];
        $.post('https://inventory/giveitemtoplayer', JSON.stringify(arr));
...
function useitem() {
    ...
    if (currentInventory == 1) {
        ...
        $.post('https://inventory/invuse', JSON.stringify(arr));
        //InventoryLog("Using item: " + name + "(" + amount + ") from " + inventoryUsedName + " | slot " + slotusing)
    }
    EndDrag(slotusing);
    closeInv(true);
...
function animateValue(id, start, end, duration) {
    ...
    timer[id] = setInterval(() => {
        current += increment;
        if (current.toFixed(2) === end.toFixed(2)) {
            clearInterval(timer[id]);
            current = end;
        }
...
function showMenu(event, menu) {
    ...
    $(menu).fadeIn();
    $(menu).css('top', (event.pageY - $(menu).height()) + 'px');
