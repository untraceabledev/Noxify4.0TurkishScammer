	...
	while true do
      ...
      if weapon == "-37975472" then
        ...
        if lastEquippedItemToRemove then
          if exports["inventory"]:hasEnoughOfItem(lastEquippedItemToRemove,1,false) then
        ...
        lastEquippedItemToRemove = nil
			...
			if throwableWeapons[weapon] and exports["inventory"]:hasEnoughOfItem(weapon,1,false) then
			...
			if selectedWeapon ~= lastWeapon and (lastWeapon == 571920712) then
				local hasItem = exports["inventory"]:hasEnoughOfItem("571920712", 1, false, true, lastEquippedInfo)
				if not hasItem then
					lastEquippedInfo = {}
				else
		...
		if IsPedPlantingBomb(ped) then
			if exports["inventory"]:hasEnoughOfItem("741814745",1,false) then
...
AddEventHandler('actionbar:setEmptyHanded', function(forceUpdateAmmo)
	prevupdate = 0
  if forceUpdateAmmo then
	  updateAmmo(true)
  end
...
function unholster1h(weaponHash, a, info)
  ...
  if weaponHash == -538741184 and info and info.componentVariant then
    local variants = {
      ["1"] = 0xAB7F6BF3,
    }
    if variants[info.componentVariant] then
  ...
  if weaponHash == 3713923289 and info and info.componentVariant then
    local variants = {
      ["1"] = 0xD28D7BA2,
    }
    if variants[info.componentVariant] then
...
function AttachmentCheck(weaponhash)
	...
	if exports["inventory"]:hasEnoughOfItem("weapon_silencer_assault", 1, false, true) then
	...
	if exports["inventory"]:hasEnoughOfItem("weapon_silencer_pistol", 1, false, true) then
	...
	if exports["inventory"]:hasEnoughOfItem("weapon_oil_silencer", 1, false, true) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, 1532150734 )
	end

	if exports["inventory"]:hasEnoughOfItem("weapon_scope", 1, false, true) then
	...
	if exports["inventory"]:hasEnoughOfItem("weapon_uzi_extended", 1, false, true) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, 0xE40F1CD2 )	
	end
	if exports["inventory"]:hasEnoughOfItem("weapon_uzi_foldstock", 1, false, true) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, 0x8FBBD54C )		
	end
	if exports["inventory"]:hasEnoughOfItem("weapon_uzi_woodstock", 1, false, true) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, 0x722C6CA6 )		
	end
	if exports["inventory"]:hasEnoughOfItem("weapon_extended", 1, false, true) then
		...
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, 0xED265A1C )
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, 0x5ED6C128 )
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, 0xD67B4F2D )
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, 0x4C04F682 )
	end
	if exports["inventory"]:hasEnoughOfItem("weapon_taser_banana", 1, false, true) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, 0x12359AF7 )	
	end
	if exports["inventory"]:hasEnoughOfItem("weapon_pickaxe", 1, false, true) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, 0xE648EB21 )	
	end
	if exports["inventory"]:hasEnoughOfItem("weapon_cutlass", 1, false, true) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, 0x415B3660 )	
	end
	if exports["inventory"]:hasEnoughOfItem("red_saber", 1, false, true) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, 0xF17E0A19 )	
	end
	if exports["inventory"]:hasEnoughOfItem("orange_saber", 1, false, true) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, 0x5B27D3 )	
	end
	if exports["inventory"]:hasEnoughOfItem("red_t_saber", 1, false, true) then
