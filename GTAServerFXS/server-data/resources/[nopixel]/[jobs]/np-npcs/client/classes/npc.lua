...
function NPC:setSetting(mode, active, settings)
    ...
    elseif mode == "randomVariation" then
        SetPedRandomComponentVariation(self.entity, 0)
        SetPedRandomProps(self.entity)
    end
end
...
function NPC:setAnimation()
  if type(self.animation) == "table" then
      local animDict = self.animation.dict
      local animName = self.animation.anim
      LoadAnimationDic(animDict)
      TaskPlayAnim(self.entity, animDict, animName, 8.0, 8.0, -1, self.animation.flag, 0, 0, 0, 0)
  end
