        ...
        while listening == listenCounter do
            if IsControlJustReleased(0, 38) then
              ...
              elseif pEvent == "townhall_court_public_records" then
                TriggerEvent("np-mdw:openMDW", { publicApp = true })
...
RegisterCommand("liceneseses", function()
  TriggerEvent("np-gov:purchaseLicenses")
end)
...
RegisterUICallback("np-gov:purchaseBusinessHandler", function(data, cb)
  ...
  Wait(1)
  local elements = {
    { name = "name", label = "Business Name", icon = "fas fa-pencil-alt" },
    { name = "propsal", label = "Short Proposal", icon = "fas fa-pencil-alt" },
  }
...
RegisterUICallback('np-gov:townhall:pdActionHandler', function(data, cb)
  ...
  local elements = {
    ...
    ['hid'] = { name = 'hid', label = 'House ID', icon = 'fas fa-house-user' },
    ['cid'] = { name = 'cid', label = 'State ID', icon = 'fas fa-id-card' },
    ['rid'] = { name = 'rid', label = 'Room ID', icon = 'fas fa-building' },
    ['bname'] = { name = 'name', label = 'Business Name', icon = 'fas fa-business-time' },
    ['bowner'] = { name = 'owner', label = 'Business Owner', icon = 'fas fa-id-badge' },
...
AddEventHandler("np-gov:townhall:LicenseVendor", function()
  local context = {
    {
      title = 'Judge Actions',
    },
    {
      title = 'License',
      description = 'Give or Remove license',
      icon = 'id-card',
      children = {
        {
          title = 'Give license',
          icon = "user-plus",
          action = 'np-gov:townhall:LicenseVendorMenu',
          key = 'give'
        },
        {
...
RegisterUICallback("np-gov:townhall:LicenseVendorMenu", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = '' } })
  local elements = {
    ['cid'] = { name = 'cid', label = 'State ID', icon = 'id-card' },
    ['licenses'] = { _type = 'select', label = 'License Type', name = 'licenses', options = {
      {
        id = "driver",
        name = "Driver License",
      },
  ...
  local actionHandlers = {
    ['give'] = function()
      ...
      local prompt = exports['np-ui']:OpenInputMenu(elements, function(values)
        return values.cid and values.cid:len() > 0 and values.licenses
      end)
      if not prompt then return end
        local result = RPC.execute("np-licenses:updateLicense", prompt.licenses, 1, prompt.cid)
        if not result then 
          return TriggerEvent('DoLongHudText', "Something went wrong" ,2)
        end
        TriggerEvent('DoLongHudText', 'Player (' .. prompt.cid .. ") received a new license." )
    end,
    ['remove'] = function()
      local elements = {
        elements['cid'],
      ...
      if not prompt then return end
      local result = RPC.execute("np-licenses:updateLicense", prompt.licenses, 0, prompt.cid)
      if not result then 
        return TriggerEvent('DoLongHudText', "Something went wrong" ,2)
      end
      TriggerEvent('DoLongHudText', "License got revoked from player (" .. prompt.cid .. ").")
