...
--                     - provide it for the default of standardVolumeOutput. Should be between
--                     - 0.1 to 1.0.
-- @param pitch        - Pitch is done by playback rate, so 1.0 is the default pitch. 0.5 is down 1 octave, 2.0 up 1 octave
--
-- Starts playing a sound locally on a single client.
------
RegisterNetEvent('InteractSound_CL:PlayOnOneUpdate')
AddEventHandler('InteractSound_CL:PlayOnOneUpdate', function(soundFile, soundVolume, pTransactionType, pitch)
    SendNUIMessage({
        transactionType     = pTransactionType or 'playSound',
        transactionFile     = soundFile,
        transactionVolume   = soundVolume,
        transactionPitch    = pitch,
    })
end)
